...
# getdatafiles ( "ftp://$host$directory",qr/$regexp/,compress option`);
#
# where $regexp is the regexp for selecting files from the ftp listing
# where compress option = 1,0: 1=recompression to gz needed, 0=recompression to gz not needed 
...
if($filename =~ /$myregexp/) {
    ...
    $filematch1 = "$filenom.Z";
    $filematch2 = "$filenom.gz";

    if ( -e $filename ) {
		$modtime = (stat("$filename"))[9];
    }
    elsif ( -e $filematch1 ){
		$modtime = (stat("$filematch1"))[9];	#gives modtime the time of last modification of current version of file
    }
    elsif ( -e $filematch2 ) {
		$modtime = (stat("$filematch2"))[9];
    }
    elsif ( -e $filenom ) {
...
# checks time of local copy again time in directory listing
		...
		if ($res->is_success()) {
			...
			if ($compress==1) {
				print "$filenom is compressing...\n";
				system("gzip -f $filenom");
			}
