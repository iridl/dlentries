...
{
IRIDB
(select cntry_name as country,* from esri2002_national where cntry_name is not null order by cntry_name) (country) openqueryby
country (Madagascar) VALUE
...
IRIDB
(select prov_na as province,* from mews_prov where prov_na is not null and country_na~'Madagascar' order by prov_na) (province) openqueryby
 province
/units /ids def
...
{
IRIDB
(select dist_na as district,* from mews_dist where dist_na is not null and country_na~'Madagascar' order by dist_na) (district) openqueryby
...
/caENACTS-last_WASP12 {
   SOURCES .DGM .Madagascar_v2 .CLM .WASP .rainfall .rfe
  [X Y]REORDER
  dup
